<template>
    <div class="side-bar flex flex-col">
        <div class="w-full logo">
            <router-link class="flex items-center w-full"
                :class="{ 'justify-center': app.sidebarCollapse, 'px-3': !app.sidebarCollapse }" :title="appTitle"
                :to="'/'">
            ...
            <el-scrollbar wrap-class="scrollbar-wrapper">
                <el-menu router unique-opened mode="vertical" :collapse="app.sidebarCollapse"
                    :default-active="defaultActive" :collapse-transition="false">
                    <side-bar-item v-for="sidebar in app.menus" :key="sidebar.path" :item="sidebar"
                        :base-path="sidebar.path" class="outer-most select-none" />
                </el-menu>
            </el-scrollbar>
    </div>
</template>
