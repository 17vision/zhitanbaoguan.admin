<template>
    <div>
            ...
            <el-form :label-position="'left'" label-width="80px">
                <el-form-item label="头像">
                    <div class="relative cursor-pointer w-[100px] h-[100px]" @click="editAvatar">
                        ...
                        <div
                            class="flex items-center justify-center w-[100px] h-[100px] pt-8 text-white bg-black/40 rounded-full absolute left-0 top-0">
        ...
        <ImageCanvas ref="cropper" title="修改用户头像" :path="form.avatar" movable :outWidth="200"
            @on-cropper-complete="onCropperComplete" />
        <!-- <ImageCropper ref="cropper" :title="'修改用户头像'" :path="form.avatar" :outWidth="200"
            @on-cropper-complete="onCropperComplete" /> -->
    </div>
</template>
...
function handleFileChange(event: any) {
    ...
    if (!file) return;
    if (file) {
        cropper.value?.open(file)
    }
    // 重要：清空 input 的值，确保下次同文件也能触发 change
    input.value = '';
}
function goBack() {
    router.back();
