          ...
          <el-table-column label="操作" align="center" fixed="right" width="180">
            <template #default="scope">
              <div class="flex gap-4 items-center justify-center">
                <!-- 上下线 -->
                <el-button
                  v-if="includes(app.routeNames, ['place.update'])"
                  link
                  size="small"
                ...
                <el-button
                  v-if="includes(app.routeNames, ['place.list'])"
                  link
                  size="small"
                  type="primary"
                  text
                ...
                <el-dropdown @command="handleCommand($event, scope.row)">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      ...
                      <el-dropdown-item
                        v-if="includes(app.routeNames, ['venue.update'])"
                        command="download"
                      >
                        下载小程序码
                      </el-dropdown-item>
                      <el-dropdown-item
                        v-if="includes(app.routeNames, ['place.introduction'])"
                        command="audio"
                        >音频列表
                      </el-dropdown-item>
                      <el-dropdown-item
                        v-if="includes(app.routeNames, ['place.delete'])"
                        command="delete"
                        divided
                        >删除
                      </el-dropdown-item>
...
function fetchData() {
  ...
  const data = { ...req }
  placesApi.list(data).then((res: Record<string, any>) => {
    if (res && res.data) {
...
const goPlacet = (item: any) => {
  if (item?.id)
    router.push({ name: 'place.introduction', query: { place_id: item.id, title: item.name } })
}
...
const goList = (value: any): void => {
  if (value && value.id) {
    router.push({
      name: 'place.list',
      query: { parent_id: value.id, venue_id: value.venue_id, title: value.name }
    })
  }
}
