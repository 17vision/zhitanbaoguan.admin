    ...
    <div>
        <div v-loading="loading" class="p-5">
            ...
            <el-table :data="tableData" class="w-full" :max-height="maxHeight">
                ...
                <el-table-column
                    v-if="includes(app.routeNames, ['organization.permission', 'organization.edit', 'organization.delete'])"
                    label="操作" align="center" fixed="right" width="200">
                    <template #default="scope">
                        <el-button v-if="includes(app.routeNames, ['organization.edit'])" link size="small"
                            type="primary" text @click="goEdit(scope.row)">编辑</el-button>
                    </template>
                </el-table-column>
            </el-table>

            <el-pagination class="mt-5" background hide-on-single-page layout="total, prev, pager, next" :total="total"
                :page-size="req.limit" v-model:current-page="req.page" @current-change="fetchData" />
        </div>
    </div>
</template>
