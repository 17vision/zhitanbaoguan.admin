    ...
    <div v-loading="$store.loading" class="p-5">
      ...
      <div class="bg-white rounded-lg shadow-md p-4">
        ...
        >
          ...
          <el-table-column
            v-if="includes(app.routeNames, ['venue.create', 'venue.update', 'venue.delete'])"
            label="操作"
            align="center"
            fixed="right"
            width="280"
          >
            <template #default="scope">
              <el-button
                v-if="includes(app.routeNames, ['venue.update']) && scope.row.status == 2"
                link
                size="small"
                type="primary"
                text
              ...
              >
              <el-button
                v-if="includes(app.routeNames, ['venue.update']) && scope.row.status == 1"
                link
                size="small"
                type="danger"
                text
              ...
              >
              <el-button
                v-if="includes(app.routeNames, ['venue.audio'])"
                link
                size="small"
                type="primary"
                text
              ...
              >
              <el-button
                v-if="includes(app.routeNames, ['venue.delete'])"
                link
                size="small"
                type="danger"
                text
...
function fetchData() {
  ...
  venue_introductionsApi.list(data).then((res: Record<string, any>) => {
    if (res && res.data) {
...
function goCreate() {
  router.push({
    name: 'venue.audio',
    query: { venue_id: router.currentRoute.value.query.venue_id }
  })
}
