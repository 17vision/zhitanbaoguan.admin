    ...
    <el-dialog :title="form.id ? '编辑导师' : '新增导师'" v-model="dialogVisible" width="600px">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="100px"
            ...
            <el-form-item label="分组" prop="resource_group_id">
                <el-select v-model="form.resource_group_id" placeholder="请选择分组">
                    <el-option v-for="item in resourceGroups" :key="item.id" :label="item.name" :value="item.id" />
                </el-select>
            </el-form-item>
...
const handleSubmit = async () => {
    try {
        ...
        if (form.value.id) {
            ...
            await api.updateResource(form.value)
            ElMessage.success('更新成功')
        } else {
            await api.createResource(form.value)
...
const getResourceGroups = async () => {
    const res = await api.getResource_groups()
