<template>
    <div v-loading="$store.loading" class="p-5">
        <div v-if="includes(app.routeNames, ['template.create'])" class="flex items-center gap-3 mb-5">
                ...
                <el-table-column label="操作" align="center" fixed="right" width="180">
                    <template #default="scope">
                        <div class="flex gap-4 items-center justify-center">
                            <!-- 上下线 -->
                            <el-button v-if="includes(app.routeNames, ['template.update'])" link size="small"
                                :type="scope.row.status == 1 ? 'danger' : 'primary'" text @click="goPublish(scope.row)">
                                {{ scope.row.status == 1 ? '下线' : '上线' }}
                            </el-button>
                            <el-button v-if="includes(app.routeNames, ['template.update'])" link size="small" type="primary"
                            ...
                            <el-dropdown @command="handleCommand($event, scope.row)">
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        <el-dropdown-item v-if="includes(app.routeNames, ['template.update'])"
                                        ...
                                        <el-dropdown-item v-if="includes(app.routeNames, ['template.delete'])"
...
const fetchData = async () => {
    try {
        const params = {
            ...req,
            combine_album_id: combine_album_id.value,
        }
        const res = await combine_templatesApi.list(params)
...
const setQrcode = async (row: any) => {
    ...
    try {
        await combine_templatesApi.qrcode(row.id)
...
const deleteFn = async (item: any) => {
    ...
    try {
        ...
        await combine_templatesApi.delete(item.id)
...
const goCreate = () => {
    router.push({
        ...
        query: { combine_album_id: combine_album_id.value },
    })
}
...
const goEdit = (item: any) => {
    if (item?.id) router.push({ name: 'template.create', query: { id: item.id } })
}
...
const goPublish = async (row: { id: string; status: number }) => {
    ...
    try {
        await combine_templatesApi.put({
            id: row.id,
            status: targetStatus,
        })
...
const handleGenerate = async () => {
    ...
    const list = tableData.value.map(item => combine_templatesApi.qrcode(item.id))
...
const confirm = async (list: any) => {
    ...
    if (list.length > 0) {
        await combine_templatesApi.sort(list)
        ElNotification.success({
            title: '排序成功',
            message: '数据排序已更新'
        })
