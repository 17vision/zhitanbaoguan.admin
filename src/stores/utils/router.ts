    ...
    function generateMenu(routes: Array<AppRoute.Route>, menus: Array<any>, prefix = '') {
        menus.forEach((item) => {
            ...
            if (item.children && item.children.length === 1 && !item.children[0]['always_show']) {
                ...
                firstChild['active_path'] = item.path
                if (config.remote) {
                    menu.meta.title = firstChild['meta']['title']
                    menu.meta.icon = firstChild['meta']['icon'] ?? item['meta']['icon']
                } else {
                    menu.meta.title = firstChild['title']
                    menu.meta.icon = firstChild['icon'] ?? item['icon']
                }
            }
